...
def add_movie(_movies):
    ...
    while True:
        ...
        if movie_name == "" or len(movie_name) > MAX_MOVIE_NAME_LENGTH:
            ...
            if movie_name in movie_storage.get_movies():
    ...
    while True:
        try:
            ...
            if MIN_YEAR > movie_year or movie_year > MAX_YEAR:
                print(f"Please enter a valid year, in the range {MIN_YEAR}-{MAX_YEAR}")
            else:
                break
    ...
    try:
        ...
        while True:
            try:
                ...
                print("Add movie error:", error)
        movie_storage.update_movie(movie_name, movie_rating)
    ...
    while True:
        try:
            ...
            if 0 <= user_input < len(FUNCTIONS):
                FUNCTIONS[user_input](movie_storage.get_movies())
            else:
